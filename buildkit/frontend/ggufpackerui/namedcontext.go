...
func (bc *Client) namedContext(ctx context.Context, name string, nameWithPlatform string, opt ContextOpt) (*llb.State, *specs.Image, error) {
	return bc.namedContextRecursive(ctx, name, nameWithPlatform, opt, 0)
}

func (bc *Client) namedContextRecursive(ctx context.Context, name string, nameWithPlatform string, opt ContextOpt, count int) (*llb.State, *specs.Image, error) {
	opts := bc.bopts.Opts
	...
	case "local":
		st := llb.Local(vv[1],
			...
			llb.SharedKeyHint("context:"+nameWithPlatform+"-"+DefaultGGUFPackerignoreName),
			llb.WithCustomName("[context "+nameWithPlatform+"] load "+DefaultGGUFPackerignoreName),
		...
		if len(dt) != 0 {
			...
			if err != nil {
				return nil, nil, errors.Wrapf(err, "failed parsing %s", DefaultGGUFPackerignoreName)
			}
		}
