...
const (
	...
	keyNameGGUFPackerfile = "ggufpackerfilekey"
)
...
type buildContext struct {
	...
	ggufpackerfile           *llb.State // override remoteContext if set
	contextLocalName         string
	ggufpackerfileLocalName  string
	filename                 string
	forceLocalGGUFPackerfile bool
}
...
func (bc *Client) initContext(ctx context.Context) (*buildContext, error) {
	...
	bctx := &buildContext{
		...
		ggufpackerfileLocalName: DefaultLocalNameContext,
		filename:                DefaultGGUFPackerfileName,
	}
	...
	if v, ok := opts[keyNameGGUFPackerfile]; ok {
		bctx.forceLocalGGUFPackerfile = true
		bctx.ggufpackerfileLocalName = v
	}
	...
	if st, ok := DetectGitContext(opts[localNameContext], keepGit); ok {
		...
		bctx.ggufpackerfile = bctx.context
		...
		if !bctx.forceLocalGGUFPackerfile {
			inputGGUFPackerfile, ok := inputs[bctx.ggufpackerfileLocalName]
			if ok {
				bctx.ggufpackerfile = &inputGGUFPackerfile
			}
		}
