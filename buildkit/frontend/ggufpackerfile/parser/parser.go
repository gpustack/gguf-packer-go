...
// but lucky for us the GGUFPackerfile isn't very complicated. This structure
...
type Node struct {
	...
	StartLine   int             // the line in the original ggufpackerfile where the node begins
	EndLine     int             // the line in the original ggufpackerfile where the node ends
	PrevComment []string
}
...
func (node *Node) canContainHeredoc() bool {
	if !command.IsHeredocDirective(strings.ToLower(node.Value)) {
		return false
	}
...
// continuation token in a GGUFPackerfile. Only ` (backtick) and \ (backslash) are
// allowed as token.
...
func init() {
	...
	dispatch = map[string]func(string, *directives) (*Node, map[string]bool, error){
		...
		command.Cat:      parseMaybeJSONToList,
		command.Cmd:      parseMaybeJSON,
		command.Copy:     parseMaybeJSONToList,
		command.Convert:  parseMaybeJSONToList,
		command.From:     parseStringsWhitespaceDelimited,
		command.Label:    parseLabel,
		command.Quantize: parseMaybeJSONToList,
	}
}
...
func newNodeFromLine(line string, d *directives, comments []string) (*Node, error) {
	...
	// Ignore invalid GGUFPackerfile instructions
	if fn == nil {
		fn = parseIgnore
	}
...
func handleScannerError(err error) error {
	...
	case bufio.ErrTooLong:
		return errors.Errorf("ggufpackerfile line greater than max allowed size of %d", bufio.MaxScanTokenSize-1)
	default:
		return err
	}
}
